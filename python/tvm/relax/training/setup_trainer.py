...
class SetupTrainer:
    ...
    def _check_mod(self, mod: IRModule):
        ...
        try:
            ...
            raise ValueError(f"SetupTrainer: The backbone module does not contain a function named {self.PREDICT_FUNC_NAME}")

        # Check function attrs
        if not self.PARAMS_NUM_ATTR_KEY in mod.attrs or not isinstance(mod.attrs[self.PARAMS_NUM_ATTR_KEY], IntImm):
            raise ValueError(f"SetupTrainer: The backbone module should has integer attribute {self.PARAMS_NUM_ATTR_KEY}")
        if not self.STATES_NUM_ATTR_KEY in mod.attrs or not isinstance(mod.attrs[self.STATES_NUM_ATTR_KEY], IntImm):
            raise ValueError(f"SetupTrainer: The backbone module should has integer attribute {self.STATES_NUM_ATTR_KEY}")
        ...
        if len(func.params) < nparam + nstate:
            raise ValueError("SetupTrainer: The number of parameters of the predict function should be no less than the number of parameters and states")

        if nstate > 0:
            if not isinstance(func.body.body, Tuple) or len(func.body.body) <= nstate:
                raise ValueError("SetupTrainer: When model state exists, the predict function should return a tuple of length more than the number of states")
