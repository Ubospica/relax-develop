...
class Trainer:
    ...
    def __init__(
        ...
        self._param_vars = self._mod[self._adjoint_fn].params[self._inputs_num:self._inputs_num + self._params_num]
        self._state_vars = self._mod[self._adjoint_fn].params[self._inputs_num + self._params_num:self._inputs_num + self._params_num + self._states_num]

        self._params: List[Optional[NDArray]] = [None] * self._params_num
        self._param_name_to_pos: Dict[str, int] = {p.name_hint: i for i, p in enumerate(self._param_vars)}

        self._states: List[Optional[NDArray]] = [None] * self._states_num
        self._state_name_to_pos: Dict[str, int] = {s.name_hint: i for i, s in enumerate(self._state_vars)}
    ...
    def _check_build(self):
        ...
        if idx_not_inited_param != -1:
            raise TVMError(f"The {idx_not_inited_param}-th parameter is not initialized before training or inference.")

        idx_not_inited_state = next((i for i, s in enumerate(self._states) if s is None), -1)
        if idx_not_inited_state != -1:
            raise TVMError(f"The {idx_not_inited_state}-th model state is not initialized before training or inference.")
    ...
    def xaiver_uniform_init_params(self):
        ...
        (2010)."""
        self._params = [
            tvm.nd.array((np.sqrt(6.0 / np.sum(p.shape)) * np.random.uniform(-1.0, 1.0, p.shape)).astype(p.dtype))
            for p in self._param_vars
        ]

    def zero_init_params(self):
        self._params = [
            tvm.nd.array(np.zeros(self._shape_expr_to_int_list(p.struct_info.shape), p.struct_info.dtype))
            for p in self._param_vars
        ]

    def zero_init_states(self):
        self._states = [
            tvm.nd.array(np.zeros(self._shape_expr_to_int_list(s.struct_info.shape), s.struct_info.dtype))
    ...
    def export_params(self) -> Dict[str, NDArray]:
        ...
        """
        return {key: self._params[pos] for key, pos in self._param_name_to_pos}
    ...
    def export_states(self) -> Dict[str, NDArray]:
        ...
        exported_dict : Dict[str, NDArray]
            The exported dictionary of parameters.
        """
        return {key: self._states[pos] for key, pos in self._state_name_to_pos}
    ...
    def update_params(self, inputs: List[Union[np.ndarray, NDArray]], targets: List[Union[np.ndarray, NDArray]]) -> NDArray:
        ...
        *inputs : Union[np.ndarray, NDArray]
        ...
        if len(inputs) != self._inputs_num:
            raise ValueError("The length of the input does not match the backbone")
        all_inputs = [tvm.nd.array(i) for i in inputs] + self._params + self._states + [tvm.nd.array(i) for i in targets]
